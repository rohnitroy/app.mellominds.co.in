...
// Session security layer (fingerprinting, idle timeout, regeneration)
...
// Auto-migrate RBAC tables on startup
async function ensureRbacSchema() {
  try {
    ...
    // Indexes
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_rbac_roles_org ON rbac_roles(org_owner_id)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_rbac_user_roles_user ON rbac_user_roles(user_id)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_rbac_audit_created ON rbac_audit_log(created_at)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_users_locked_until ON Users(locked_until) WHERE locked_until IS NOT NULL`);
    ...
    console.log('✅ RBAC schema verified');
  } catch (err) {
    console.error('⚠️  RBAC schema migration warning:', err.message);
  }
}
...
httpServer.listen(PORT, async () => {
  ...
  await ensureRbacSchema(); // Initialize RBAC tables
